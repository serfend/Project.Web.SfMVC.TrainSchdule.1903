...
namespace TrainSchdule.Controllers
{
    ...
    public class DWZController : Controller
    {
        ...
        public async Task<IActionResult> RemoveDwz([FromRoute] string url)
        {
            ...
            var m = await dWZServices.Load(url).ConfigureAwait(true);
            if (m == null) return new JsonResult(ActionStatusMessage.StaticMessage.ResourceNotExist);
            var permit = userActionServices.Permission(c, ApplicationPermissions.Resources.ShortUrl.Item, PermissionType.Write, new List<string>{ m.CreateBy.CompanyInfo.CompanyCode }, "移除短网址", out var failCompany);
        ...
        {
            var c = currentUserService.CurrentUser;
            var permit = userActionServices.Permission(c, ApplicationPermissions.Resources.ShortUrl.Item, PermissionType.BanWrite, new List<string> { null }, "创建短网址", out var failCompany);
            if (!permit) return new JsonResult(ActionStatusMessage.Account.Auth.Invalid.Default);
            try
            {
        ...
        public async Task<IActionResult> QueryStatistics([FromRoute] string key, [FromBody] QueryDwzStatisticsViewModel model)
        {
            ...
            if (m == null) return new JsonResult(ActionStatusMessage.StaticMessage.ResourceNotExist);
            if (!userActionServices.Permission(c, ApplicationPermissions.Resources.ShortUrl.Item, PermissionType.Read,new List<string> { m.CreateBy.CompanyInfo.CompanyCode }, "查询短网址统计", out var failCompany)) return new JsonResult(ActionStatusMessage.Account.Auth.Invalid.Default);
