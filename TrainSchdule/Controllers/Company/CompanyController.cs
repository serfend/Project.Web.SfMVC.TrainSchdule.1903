...
namespace TrainSchdule.Controllers
{
    ...
    public partial class CompanyController : Controller
    {
        ...
        /// <param name="asManage">是否按编制查询</param>
        /// <returns></returns>
        [HttpGet]
        [AllowAnonymous]
        [ProducesResponseType(typeof(AllMembersDataModel), 0)]
        public IActionResult Members(string code, bool asManage, int page, int pageSize = 100)
        {
            var companyInfo = _currentUserService.CurrentUser?.CompanyInfo;
            code ??= (asManage ? companyInfo?.CompanyOfManageCode : companyInfo?.CompanyCode);
            var list = code == null ? new Tuple<IQueryable<User>, int>(null, 0) : _companyManagerServices
                .GetMembers(code, asManage)
