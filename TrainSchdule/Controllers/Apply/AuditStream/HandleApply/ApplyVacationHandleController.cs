...
namespace TrainSchdule.Controllers.Apply.AuditStream.HandleApply
{
    ...
    public class ApplyVacationHandleController : Controller
    {
        ...
        /// 
        /// </summary>
        public ApplyVacationHandleController(IUsersService usersService, ICurrentUserService currentUserService, IUserActionServices userActionServices, IAuditStreamServices auditStreamServices, IGoogleAuthService googleAuthService, ApplicationDbContext context, IApplyVacationService applyService)
        {
        ...
        private void CheckApplyModelAndDoTask(string id, Action<DAL.Entities.ApplyInfo.Apply, string> callBack, bool needPermission = true)
        {
            ...
            var apply = context.Applies.FirstOrDefault(i => i.Id == gid) ?? throw new ActionStatusMessageException(ActionStatusMessage.ApplyMessage.NotExist);
            ...
            {
                var permit = userActionServices.Permission(currentUser, ApplicationPermissions.Apply.Vacation.Detail.Item, PermissionType.Write, new List<string> { apply.BaseInfo.CompanyCode }, "执行休假申请的操作", out var failCompany);
