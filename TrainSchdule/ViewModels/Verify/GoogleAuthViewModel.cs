...
namespace TrainSchdule.ViewModels.Verify
{
	...
	public static class GoogleAuthExtension
	{
		...
		/// <returns></returns>
		public static bool Verify(this GoogleAuthDataModel model, IGoogleAuthService authService, string currentUserId) => model?.AuthByUserID == currentUserId || (model != null && authService.Verify(int.Parse(model.Code), model?.AuthByUserID));
