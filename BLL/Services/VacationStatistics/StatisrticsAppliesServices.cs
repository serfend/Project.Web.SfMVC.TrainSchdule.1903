...
namespace BLL.Services.VacationStatistics
{
	public class StatisrticsAppliesServices : IStatisrticsAppliesServices
	{
		...
		{
			var list = _context.StatisticsCompleteApplies.Where(s => s.CompanyCode.Contains(companyCode)).Where(s => s.Target >= vStart).Where(s => s.Target <= vEnd);
		...
		{
			var list = _context.StatisticsNewApplies.Where(s => s.CompanyCode.Contains(companyCode)).Where(s => s.Target >= vStart).Where(s => s.Target <= vEnd);
