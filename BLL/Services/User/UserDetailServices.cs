...
namespace BLL.Services
{
    public partial class UsersService
    {
        ...
        public int CheckAuthorizedToUser(User authUser, User modifyUser)
        {
            ...
            var targetCompanyOfManage = modifyUser.CompanyInfo.CompanyOfManageCode;
            // 判断是否有管理此单位的权限，并且级别高于此单位至少1级
            var result1 = targetCompany == null ? 10 : myManages.Max(m => targetCompany.StartsWith(m.Code) ? targetCompany.Length - m.Code.Length : -1);
            var result2 = targetCompanyOfManage == null ? 10 : myManages.Max(m => targetCompanyOfManage.StartsWith(m.Code) ? targetCompanyOfManage.Length - m.Code.Length : -1);
